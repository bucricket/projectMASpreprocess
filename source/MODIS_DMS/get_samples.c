...
int savePureSamples(SENSOR *spec, SENSOR *th, INPUT_PARS *pars) 
{
  ...
  for(i=0; i<th->nrows; i++) {
	...
	if(bn==spec->nbands) {
	  ...
	  if(ave_cv < pars->CV_TH) {	
	    ...
	    fprintf(th_sample, "%6.1f, ", ndvi);
	    /*if(ave[0]+ave[1] != 0) 
	      ndvi = (ave[1]-ave[0])/(ave[1]+ave[0]);
            else
	    ndvi = -1.0;*/
  ...
  fprintf(th_names, "attributes excluded: irow, icol, ndvi\n\n"); 
  ...
  printf("\n\toriginal: nrows=%4d; ncols=%4d; res=%6.3f", spec->nrows, spec->ncols, spec->res);
  printf("\n\tcoarse:   nrows=%4d; ncols=%4d; res=%6.3f (resize factor =%d)", th->nrows, th->ncols, th->res, scale);
