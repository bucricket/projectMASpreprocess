 ...
 * combined versions on 10/29/2015
 **/
...
int getSensorMetaInfo(SENSOR *shortw, SENSOR *oth, SENSOR *cth, SENSOR *sth, INPUT_PARS *pars) 
{
  ...
  if((oth->angle=fopen(pars->org_th_view_angle, "rb"))==NULL) {
    printf("Can not open MODIS LST view angle file %s\n", pars->org_th_view_angle);
  }
  ...
  alloc_2dim_contig((void ***) (&oth->vzn), oth->nrows, oth->ncols, sizeof(float));
...
int loadThermal(SENSOR *th)
{
  ...
  for(i=0; i<th->nrows; i++) {
	...
	th->fdata[i][j] = one_row_tiff[j];
      ...
      /* read view angle array from binary file */
      if(th->angle != NULL)
	fread(th->vzn[i], sizeof(float), th->ncols, th->angle);
      else  
	/* if angle file doesnot exist then sharpen TIR (vzn=90) anyway */
	for(j=0; j<th->ncols; j++)
	  th->vzn[i][j] = 90.0;   
    }
  }
  ...
  if(strcasecmp(th->fileType, "binary")==0) {
    rewind(th->fp[0]);
    if(th->angle != NULL) rewind(th->angle);
  }
  free(one_row_tiff);
  return SUCCESS;
}
...
int resize(SENSOR *oth, SENSOR *cth)
{
  ...
  printf("\n\toriginal: nrows=%4d; ncols=%4d; res=%6.3f", oth->nrows, oth->ncols, oth->res);
  printf("\n\tcoarse:   nrows=%4d; ncols=%4d; res=%6.3f (resize factor =%d)", cth->nrows, cth->ncols, cth->res, scale);
...
int cleanUpSensor(SENSOR *shortw, SENSOR *oth, SENSOR *cth, SENSOR *sth)
{
  ...
  if(strcasecmp(oth->fileType, "binary") == 0) { 
    fclose(oth->fp[0]);
    if(oth->angle != NULL) fclose(oth->angle);
  }
...
{  
  ...
  int ii, jj, si, sj, flag;
  float x, y, dx, dy, v[2][2], s_diff, w, vzn;
  float  **diff, **stdev;
  double sum;
  float tsum1, tsum2;
  ...
  alloc_2dim_contig((void ***) (&stdev), cth->nrows, cth->ncols, sizeof(float));
  ...
  for(i=0; i<cth->nrows; i++)
	...
	for(n=j*scale; n<(j+1)*scale; n++) {
	  ...
	  if(sth->fdata[m][n]>=sth->range[0]&&sth->fdata[m][n]<=sth->range[1]) {
	    sum += pow(sth->fdata[m][n], 4);
	    num++;
	    tsum1 += sth->fdata[m][n];
	    tsum2 += pow(sth->fdata[m][n],2);
	  }
	}
      if(num != scale*scale || cth->fdata[i][j]<cth->range[0] || cth->fdata[i][j]>cth->range[1]) {
	diff[i][j] = FILLV;
	stdev[i][j] = 0;
      }
      else {
	...
	stdev[i][j] = tsum2/num - tsum1*tsum1/num/num;
      }
      /* printf("tsum1=%10.5f tsum2=%10.5f num=%d stdev=%10.5f\n", tsum1, tsum2, num, stdev[i][j]);*/
    } /* end of image loop */
  ...
  for(i=0; i<sth->nrows; i++) {
	...
	for(ii=0; ii<2; ii++)
	  for(jj=0; jj<2; jj++)
	    if(si+ii>0&&si+ii<cth->nrows&&sj+jj>0&&sj+jj<cth->ncols&&diff[si+ii][sj+jj]!=FILLV)
...
#endif
      if(sth->fdata[i][j]<oth->range[0]||sth->fdata[i][j]>oth->range[1]||s_diff==FILLV)
	sth->fdata[i][j] = FILLV;
      else
	sth->fdata[i][j] += s_diff;

      /* if no model prediction (missing spectral info) but has a valid original LST then use it (5/20/2014) */
      /* if predictions has same values (uniform) within the corase pixel, then use original LST (5/21/2014) */ 
      /* replaces missing in the final sharpened and energy conserved image with original value if valid
	 (lost in energy conservation due to bad neighbor pixels) (8/1/2014) 
	 - lost version control, combined on 10/29/2015 */
      if ((sth->fdata[i][j] == FILLV && oth->fdata[i][j] != FILLV) || stdev[m][n]<0.01)
	sth->fdata[i][j] = oth->fdata[i][j];	

      /* compute weight on view angle (vzn) - Feng (11/11/2015) 
	 use original LST (szn<20); 
	 use linear weight to combine two (20-40); 
	 use sharpened LST (szn>40) */
      vzn = oth->vzn[i][j];
      if(vzn < 20) w = 0.0;
      else if(vzn > 40) w = 1.0;
      else w = (vzn-20)/20.0;
      // if(i==310&&j==245) printf("%f %f %f %f\n", vzn, w, oth->fdata[i][j], sth->fdata[i][j]);
      if(sth->fdata[i][j] != FILLV && oth->fdata[i][j] != FILLV)
	sth->fdata[i][j] = sth->fdata[i][j] * w + oth->fdata[i][j] * (1-w);	
    }
  }
