...
ItemCount	*Freq;			/* discrete value frequencies */
...
/*  MaxItem is set to the highest numbered case.			 */
/*									 */
/*************************************************************************/
...
/*   -------  */
{
	     ...
	     ( AllowUnknownTarget || CVal(DVec, ClassAtt) != UNKNOWN ) )
	{
	    ...
	    if ( MaxItem > 0 && MaxItem % 100 == 0 )
	    {
		NotifyNumber("Case", MaxItem);
	    }
#endif
#endif
	    MaxItem++;
    ...
    ForEach(i, 0, MaxItem)
    {
	AnyUnknown |= ReplaceUnknowns(Item[i], AttMsg);
    }
    ...
    if ( Train )
    {
	/*  Generate global max and min values  */
	...
	ForEach(i, 0, MaxItem)
	{
	    if ( (Val = Class(Item[i]) ) > Ceiling ) Ceiling = Val;
	    if ( Val < Floor ) Floor = Val;
	}

	Range = Ceiling - Floor;
	Ceiling += EXTRAP * Range;
	Floor   -= EXTRAP * Range;
    }
...
/*  --------------  */
{
	...
	case BrSubset:
	    ...
	    fprintf(Of, " " T_ElementOf " {");
	    Col = CharWidth(AttName[Att]) + CharWidth(T_ElementOf) + 11;
		...
		if ( In(pv, C->Subset) )
		{
		    ...
		    if ( Col + Entry >= Width )
		    {
			Col = CharWidth(AttName[Att])
			      + CharWidth(T_ElementOf) + 11;
			fprintf(Of, "\n%*s", Col, "");
		    }
...
/*    -----------------  */
{
    ...
    return ( Weight == 0 ? GlobalMean : Sum / Weight );
}
...
#define  MAXN	       50	/* max number allowing for ties */
#define	 DPREC	       10	/* distance precision */
...
/*   -------------------  */
{
    ...
    Instance    = Alloc(MaxItem+1, Description);
    MaxInstance = MaxItem;
    ForEach(i, 0, MaxItem)
    {
	Instance[i] = Item[i];
    ...
    ValFreq = Alloc(MaxDiscrVal+1, ItemCount);

    GNNEnv.AttMinD = Alloc(MaxAtt+1, float);

    /*  Construct the reference and determine distances  */

    Ref = Alloc(MaxAtt+1, AttValue);
    ForEach(Att, 1, MaxAtt)
    {
	if ( Continuous(Att) )
	{
	    CVal(Ref, Att) = AttMean[Att] - 2.5 * AttSD[Att];
	}
	else
	{
	    DVal(Ref, Att) = 2;
    ...
    {
	DRef(Instance[i]) = Distance(Instance[i], Ref, 1E38);
    }

    KDTree = BuildIndex(1, 0, MaxItem);

    Free(Tested);				Tested = Nil;
    Free(ValFreq);				ValFreq = Nil;

    /*  Tabulate values predicted by ruleset  */

    RSPredVal = Alloc(MaxItem+1, float);
...
/*    ----------  */
{
    ...
    return AverageNeighbors(Cttee, Case, E);
}
...
/*   -------------  */
{
    ...
    Dist = rint(DPREC *
		Distance(Case, Instance[Saved], *E->WorstBest + 0.5 / DPREC))
	   / DPREC;
...
/*   --------------------  */
{
    ...
    DRef(Case) = Distance(Case, Ref, 1E38);
...
/*    ----------  */
{
    ...
    if ( Lp > Fp )
    {
	MinDist = MaxDist = DRef(Instance[Fp]);
	ForEach(i, Fp+1, Lp)
	{
	    if ( (Dist = DRef(Instance[i])) < MinDist )
	    {
		MinDist = Dist;
	    }
	    else
	...
	Node->MinDRef = MinDist;
	Node->MaxDRef = MaxDist;
	...
	ForEach(Att, 1, MaxAtt)
	{
	  if ( Skip(Att) || Att == ClassAtt || (Tested[Att] && Discrete(Att)) )
		...
		else
		{
		    ForEach(v, 2, MaxAttVal[Att])
		    {
			ExpDist += ValFreq[v] * (Items - ValFreq[v]) * 2.0;
		    }
		}

		ExpDist /= (MaxAttVal[Att] - 1) * Items * Items;
	    }
	...
	ForEach(v, 1, MaxAttVal[BestAtt])
	{
	    ...
	    Node->SubIndex[v] = BuildIndex(BestAtt+1, Fp, Kp-1);
	    Fp = Kp;
	}
	...
	else
	{
	    ...
	    Node->SubIndex[1] = BuildIndex(BestAtt+1, Fp, Xp-1);
	    Node->SubIndex[2] = BuildIndex(BestAtt+1, Xp, Kp-1);
	    Node->SubIndex[3] = BuildIndex(BestAtt+1, Kp, Lp);
	}
    }
...
/*   ---------  */
{
    ...
    if ( Max(Node->MinDRef - DRef(Case), DRef(Case) - Node->MaxDRef) <=
	...
	ForEach(v, 1, Forks)
	{
	    ...
	    E->AttMinD[Att] =
			...
			Ordered(Att) ? fabs(v - First) / (MaxAttVal[Att] - 1) :
			2.0 / (MaxAttVal[Att] - 1) );
...
/*  -----------  */
{
    ...
    {
	RoundErr = fabs(((double) rint(Val * Mult)) / Mult - Val);
...
/*  --------  */
{
	...
	if ( CVal(Item[i], Att) != UNKNOWN &&
	     ! NotApplic(Item[i], Att) &&
	     (ThisDenom = Denominator(CVal(Item[i], Att))) > Denom )
	{
	    Denom = ThisDenom;
	}
    }
